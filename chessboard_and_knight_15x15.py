...
solution = [[-1 for _ in range(max_board)] for _ in range(max_board)]
...
def dfs_knight_tour(y, x, current_step, board, backtrack_count, solution):
    ...
    #end condition if it is possible to move form position (number_of_moves - 1) to field [2,1] then success solution was found
    if current_step == (number_of_moves - 1):
        if (2, 1) in knight_moves(y, x):
            ...
            return True
        # if we cant move from position (number_of_moves - 1) to field [2,1] the solution was not found
        return False
    # moves = all moves
...
board = [[-1 for _ in range(max_board)] for _ in range(max_board)]
board[0][0] = 0 # start point 0,0
board[2][1] = number_of_moves # last step
